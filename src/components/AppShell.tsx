...
export function AppShell({ children, title }: { children: ReactNode; title?: string }) {
  ...
  return (
    ...
    <div className="min-h-dvh overflow-x-clip pb-[calc(7rem+env(safe-area-inset-bottom))] lg:pb-0 lg:pe-72">
      ...
      <header className="fixed top-0 inset-x-0 z-40 px-4 pt-[calc(0.75rem+env(safe-area-inset-top))] pb-2 lg:hidden">
        {/* طبقة البلور المقنّع — تُعطَّل على الجوال تلقائياً عبر CSS */}
        <div className="top-veil-blur" aria-hidden="true" />
        {/* طبقة التدرّج اللوني — تعمل دائماً، بما في ذلك الجوال */}
        <div className="top-veil" aria-hidden="true" />
          ...
          <div className="relative flex items-stretch justify-between">
            ...
            {NAV.map((n) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={`size-[18px] ${pillReady ? "transition-transform duration-[200ms] ease-[cubic-bezier(0.34,1.35,0.64,1)]" : ""} ${active ? "scale-110 -translate-y-px" : ""}`}
