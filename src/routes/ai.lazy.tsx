...
import { Send, Sparkles, Loader2, MoreVertical, Plus, Trash2, Pencil, Share2, Flag, X, Search, Check, XCircle } from "lucide-react";
...
function AIPage() {
  ...
  useEffect(() => {
    if (sideOpen) {
      requestAnimationFrame(() => setSideVisible(true));
      return;
    }
  ...
  const newChat = () => { setMessages([]); setConversationId(null); closeSidebar(); setReportSent(false); };
  ...
  return <AppShell title="مساعد عبوسي">
    <div className="animate-reveal flex items-center justify-between gap-3 mb-4">
      ...
      <button onClick={() => setSideOpen(true)} className="p-2 rounded-full text-muted-foreground hover:bg-surface-2/70" aria-label="سجل المحادثات"><MoreVertical className="size-5" /></button>
    </div>
    ...
    <div className="space-y-3 pb-32">
      {!messages.length && <div className="glass p-4 rounded-2xl text-sm leading-relaxed">أهلاً، أنا عبوسي. اسألني عن أي درس، وسأشرحه لك بكل بساطة.</div>}
      {messages.map((m) => <div key={m.id} className={`max-w-[85%] p-3.5 rounded-2xl text-sm leading-relaxed animate-reveal ${m.role === "assistant" ? "glass mr-0 ml-auto rounded-tr-md" : "bg-accent text-accent-foreground ml-0 mr-auto rounded-tl-md"}`}>{getImageParts(m).map((part, index) => <img key={index} src={part.url} alt="رسالة الطالب" className="max-w-full rounded-xl mb-2" />)}{m.role === "assistant" ? <div className="prose prose-sm prose-invert max-w-none [&_p]:my-1.5 [&_ul]:list-disc [&_ul]:pr-5 [&_ol]:list-decimal [&_ol]:pr-5 [&_strong]:font-bold"><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ a: ({ href, children }) => <a href={href} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-full bg-accent/15 text-accent px-2 py-0.5 no-underline text-xs"><span className="size-1.5 rounded-full bg-current" />{children}</a> }}>{getText(m)}</ReactMarkdown></div> : getText(m) && <span className="whitespace-pre-wrap">{getText(m)}</span>}</div>)}
    ...
    <div className="fixed bottom-[calc(7rem+env(safe-area-inset-bottom))] left-4 right-4 z-40 lg:bottom-8 lg:left-72 lg:right-0 lg:px-10"><div className="lg:max-w-2xl lg:mx-auto">{attachment && <div className="glass rounded-xl px-3 py-2 mb-2 text-xs flex items-center justify-between"><span className="truncate">المرفق: {attachment.name}</span><button onClick={() => setAttachment(null)}><X className="size-3" /></button></div>}<form onSubmit={onSubmit} className={`glass-strong relative flex items-center gap-1 rounded-2xl p-1 shadow-glass ${isGuest ? "opacity-80" : ""}`}><label className={`p-1 text-muted-foreground ${isGuest ? "cursor-not-allowed" : "hover:text-foreground cursor-pointer"}`} title="إرفاق ملف أو صورة"><input type="file" className="hidden" accept="image/*,video/*,audio/*,.pdf,.doc,.docx,.txt" onChange={(e) => void onFile(e.target.files?.[0])} disabled={isGuest} /><Plus className="size-5" /></label><input ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} placeholder={isGuest ? "سجّل الدخول لاستخدام عبوسي" : "اكتب سؤالك لعبوسي…"} className="min-w-0 flex-1 bg-transparent px-2.5 py-2 pe-11 text-sm focus:outline-none" disabled={isLoading || isGuest} /><button type="submit" disabled={isLoading || uploading || isGuest || (!input.trim() && !attachment)} className="absolute end-1.5 top-1.5 size-9 rounded-lg bg-accent text-accent-foreground grid place-items-center disabled:opacity-50 active:scale-95" aria-label="إرسال"><Send className="size-4 rotate-180" /></button></form></div></div>

    {sideOpen && <div className={`fixed inset-0 z-50 bg-black/40 transition-opacity duration-400 ${sideVisible ? "opacity-100" : "opacity-0"}`} onClick={closeSidebar}><aside className={`absolute left-0 top-0 bottom-0 w-[min(88vw,360px)] bg-background p-4 shadow-2xl transition-transform duration-400 ease-out ${sideVisible ? "translate-x-0" : "-translate-x-full"}`} dir="rtl" onClick={(e) => e.stopPropagation()}><div className="flex items-center justify-between mb-5"><button onClick={closeSidebar} className="size-9 glass rounded-xl grid place-items-center"><X className="size-4" /></button><div className="font-bold">سجل المحادثات</div><button onClick={newChat} className="size-9 rounded-xl bg-accent text-accent-foreground grid place-items-center" title="محادثة جديدة"><Plus className="size-4" /></button></div><div className="space-y-2 overflow-y-auto max-h-[70vh]">{history.map((item) => <div key={item.id} onPointerDown={() => { longPressRef.current = setTimeout(() => { const action = window.prompt("اكتب: تعديل أو حذف أو مشاركة أو تبليغ"); if (action === "تعديل") void renameConversation(item); if (action === "حذف") void deleteConversation(item); if (action === "مشاركة") void shareConversation(); if (action === "تبليغ") setReporting(true); }, 650); }} onPointerUp={() => { if (longPressRef.current) clearTimeout(longPressRef.current); }} className="glass rounded-xl p-3 text-sm cursor-pointer"><div className="font-bold truncate">{item.title}</div><div className="text-[10px] text-muted-foreground mt-1">اضغط مطولًا للتعديل أو الحذف أو المشاركة أو التبليغ</div></div>)}{!history.length && <div className="text-sm text-muted-foreground text-center py-8">لا توجد محادثات محفوظة</div>}</div></aside></div>}
