...
function AIPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => { e.preventDefault(); const t = input.trim() || (attachment ? `أرسل هذا المرفق إلى شخص سأحدده` : ""); if (!t || isLoading || uploading) return; setInput(""); const planResponse = await fetch("/api/agent", { method: "POST", headers: { "content-type": "application/json", ...(await authHeaders()) }, body: JSON.stringify({ request: t, conversationId, attachment }) }); if (planResponse.ok) { const planned = await planResponse.json(); if (planned.plan?.needsConfirmation) { setPendingPlan({ runId: planned.runId, summary: planned.plan.summary, steps: planned.plan.steps }); return; } } await sendMessage({ text: t }); setAttachment(null); if (!isTouchDevice()) inputRef.current?.focus(); };
  const confirmPlan = async () => { if (!pendingPlan) return; const response = await fetch("/api/agent", { method: "PATCH", headers: { "content-type": "application/json", ...(await authHeaders()) }, body: JSON.stringify({ runId: pendingPlan.runId, confirmed: true }) }); setPendingPlan(null); setAttachment(null); if (response.ok) await sendMessage({ text: "تم التأكيد. نُفذت الخطة، وأخبرني بالنتيجة." }); else window.alert("تعذّر تنفيذ الخطة"); };
